...
8. 平台采集链路必须封装为可复用功能函数，Adapter、API、Worker、导入任务共用同一实现。
...
## 3. 阶段 0：平台采集可行性验证与 Adapter 定型

目标：确认真实采集链路可行，并将可行链路沉淀为正式 Adapter。

交付物：

1. 微博采集 spike：
   - 已验证用户授权登录态可复用。
   - 已验证 `ajax/statuses/mymblog` 可获取列表。
   - 已验证详情页可补全截断正文。
   - 已验证图片 URL 可提取和下载。
   - 已沉淀为 `WeiboAdapter` 与 `collect_weibo_posts()`。
2. 雪球采集 spike：
...
1. 微博能通过正式 `collect_weibo_posts()` 返回 `PageResult`。
2. 微博能得到用于去重的 `platform_post_id`。
3. 微博能构造内容详情原文链接。
4. 微博能提取图片原始 URL 或可下载 URL。
5. 雪球完成同级别 spike 或明确第一版降级策略。
